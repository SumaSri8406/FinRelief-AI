...
import { ShieldAlert, ArrowLeft, Save, RefreshCw } from 'lucide-react';
...
export const EditLoan = () => {
  ...
  useEffect(() => {
    const fetchLoan = async () => {
      ...
      setError('');
      try {
        const loan = await loanService.get(id);
        if (loan) {
        ...
        setError('Failed to retrieve details of this loan. Verify backend availability.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Failed to modify loan. Verify fields are valid.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: '700px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '1.5rem' }}>
        ...
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
            Adjust the terms, balance, or status parameters of this registered loan contract.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 0', gap: '1rem', color: 'var(--text-secondary)' }}>
          ...
          <span>Fetching loan details from server...</span>
        </div>
      ) : (
        /* Form Card */
